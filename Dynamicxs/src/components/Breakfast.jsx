...
function Breakfast() {
  ...
  const [newProduct, setNewProduct] = useState({ name: '', description: '', price: '', image: null, category: 'breakfast' });
  const [productImage, setProductImage] = useState(null);
  const navigate = useNavigate();

  useEffect(() => {
    const token = localStorage.getItem('token');
    fetch('http://localhost:8080/product?category=breakfast', {
      method: 'GET',
      headers: {
  ...
  const handleFormSubmit = (e) => {
    ...
    .then(data => {
      ...
      setIsPopupOpen(false);
      setNewProduct({ name: '', description: '', price: '', image: null, category: 'breakfast' });
          ...
          <div className="flex gap-3 mb-5">
            ...
            <div className="flex flex-col justify-start -ml-20">
              ...
              {products.map((product, index) => (
                <img key={index} src={`data:image/jpeg;base64,${product.image}`} alt={product.name} className="ml-40" style={{ width: '150px', height: '150px', marginTop: index > 0 ? '10px' : '0' }} />
              ))}
            </div>
            <section>
