...
function Lunch() {
  ...
  const [newProduct, setNewProduct] = useState({ name: '', description: '', price: '', image: null, category: 'lunch' });
  const [productImage, setProductImage] = useState(null);
  const navigate = useNavigate();

  useEffect(() => {
    const token = localStorage.getItem('token');
    fetch('http://localhost:8080/product?category=lunch', {
      method: 'GET',
      headers: {
  ...
  const handleFormSubmit = (e) => {
    ...
    .then(data => {
      ...
      setIsPopupOpen(false);
      setNewProduct({ name: '', description: '', price: '', image: null, category: 'lunch' });
      ...
      <div className="flex">
        ...
        <div className="flex flex-col items-center py-11 bg-amber-100 shadow-sm flex-grow">
          ...
          <div className="ml-4">
            ...
            <button className={`px-2 py-2 text-2xl rounded-xl shadow-sm mb-4 ml-6 custom-button ${location.pathname === "/lunch" ? 'bg-red-500' : 'bg-orange-200'}`}
              onClick={() => {
                toggleSidebar();
                navigate("/lunch");
              }}>
          ...
          <div className="flex gap-3 mb-5">
            ...
            <div className="flex flex-col justify-start -ml-20">
              ...
              {products.map((product, index) => (
                <img key={index} src={product.image} className="ml-40" style={{ width: '150px', height: '150px', marginTop: index > 0 ? '10px' : '0' }} />
              ))}
            </div>
            <section>
