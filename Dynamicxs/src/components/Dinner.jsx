...
function Dinner() {
  ...
  const [newProduct, setNewProduct] = useState({ name: '', description: '', price: '', image: null, category: 'dinner' });
  const [productImage, setProductImage] = useState(null);
  const navigate = useNavigate();

  useEffect(() => {
    const token = localStorage.getItem('token');
    fetch('http://localhost:8080/product?category=dinner', {
      method: 'GET',
      headers: {
  ...
  const handleFormSubmit = (e) => {
    ...
    .then(data => {
      ...
      setIsPopupOpen(false);
      setNewProduct({ name: '', description: '', price: '', image: null, category: 'dinner' });
